...
subplot(2, 1, 1)
hold on;
plot(t, psi*rad2deg);           %heading
plot(t, (psi + beta)*rad2deg);  %course
plot(t, chi_d*rad2deg);         %desired course
l = legend('$\psi$', '$\chi$', '$\chi_d$');
...
subplot(2, 1, 2);
hold on;
plot(t, beta*rad2deg);          %sideslip
l = legend('$\beta$');
set(l, 'interpreter', 'latex');
xlabel('time [s]');
ylabel('crab angle [deg]');
grid on;
